...
/******** OWNER NUMBER**********/
const ownerNumber = ["6283873394995@s.whatsapp.net"] 
/************************************/
...
const client = new WAConnection()
   ...
   console.log(color('[','white'),color('∆','red'),color(']','white'),color('qr already scan.subscrib','white'),color('YOU','red'),color('TUBE','white'),color('ampibi gaming','yellow'))
})
...
case 'randomhentai':
            ...
            case 'asupan':
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				    try {
				    reply(ind.wait())
						anu = await fetchJson(`https://mieayam-kering.herokuapp.com/api/asupan?apikey=freeKeY`)
						buffer = await getBuffer(anu.result)
						client.sendMessage(from, buffer, video, {quoted: mek, caption: ' *Nih gan* '}) 
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERROR* ❌')
					}
...
case 'delete':
					...
					case 'wasted':
			              	...
			              	if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        ...
                                        ranp = getRandom('.png')
                                        rano = getRandom('.webp')
                                        anu1 = `https://mieayam-kering.herokuapp.com/api/img/wasted?url=${teks}&apikey=freeKeY`
					...
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					anu = await fetchJson(`https://mieayam-kering.herokuapp.com/api/katailham?apikey=freeKeY`, {method: 'get'})
					reply(anu.result)
					await limitAdd(sender)
					break
         case 'loli':
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				    try {
				    reply(ind.wait())
						res = await fetchJson(`https://tobz-api.herokuapp.com/api/randomloli?apikey=BotWeA`)
...
case 'waifu':
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				    try {
				    reply(ind.wait())
						res = await fetchJson(`https://mieayam-kering.herokuapp.com/api/waifu?apikey=freeKeY`)
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				    ...
				    reply(ind.wait())
						...
						buffer = await getBuffer(anu.result)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Ingat! Cintai Nekomu'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERROR* ❌')
					}
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				    try {
				    reply(ind.wait())
						anu = await fetchJson(`https://tobz-api.herokuapp.com/api/randomshota?apikey=BotWeA`)
						buffer = await getBuffer(anu.result)
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				    try {
				    reply(ind.wait())
						dark = await fetchJson(`https://api.zeks.xyz/api/darkjokes?apikey=apivinz`)
						buffer = await getBuffer(dark.result)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Ketawa dosa!!!'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERROR* ❌')
					}
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				    try {
				    reply(ind.wait())
						meme = await fetchJson(`https://api.zeks.xyz/api/memeindo?apikey=apivinz`)
        ...
        if (isPrem) return reply(ind.prem())
			...
			if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				/*if (!isPublic) return reply(mess.only.publicG)*/
                ps = `${body.slice(11)}`
                  anu = await fetchJson(`https://api.zeks.xyz/api/sgplay?apikey=BotWeA&q=${ps}`, {method: 'get'})
                  store = '======================\n'
...
• *Link Apk*: ${ply.url}\n===================°]\n`
                  ...
                  client.sendMessage(from, buffer, image, {quoted: mek, caption: store.trim(), text, tescuk, cr})
                  break
case 'ban':
					...
					bannd = ban.push(8)
					ban.push(`${bannd}@s.whatsapp.net`, 1)
...
case 'nekopoi':
				...
				anu = await fetchJson (`https://freerestapi.herokuapp.com/api/v1/nekopoi?url=https://freerestapi.herokuapp.com/api/v1/nekopoi?url='https://nekopoi.care/sexfriend-gakuen-episode-1-subtitle-indonesia/`)
...
case 'porn':
			   ...
			   reply(mess.wait)
                    ...
                    teks = body.slice(9)
                    anu = await fetchJson(`https://freerestapi.herokuapp.com/api/v1/nekopoi?url=${teks}`, {method: 'get'})
                    teks = `===============\n`
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					client.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, video, {mimetype: 'video/tiktok', filename: `${anu.title}.tiktok`, quoted: mek})
					await limitAdd(sender)
				break
				case 'ytmp3':
                    if (!isRegistered) return reply(ind.noregis())
					...
					anu = await fetchJson(`https://api.zeks.xyz/api/ytmp3?url=${args[0]}&=apivinz`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					teks = `*Title* : ${anu.title}\n*Size* : ${anu.size}`
            ...
            break
                ...
                addKoinUser('6285762102148@s.whatsapp.net', fee)
...
case 'caklontong':
        ...
        anu = await fetchJson('https://mieayam-kering.herokuapp.com/api/caklontong?apikey=freeKeY', {method: 'get'})
					setTimeout( () => {
					client.sendMessage(from, '*Jawaban :* '+anu.soal.jawaban+{quoted: mek}) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
					client.sendMessage(from, '10 Detik lagi', text) // ur cods
					}, 20000) // 1000 = 1s,
					setTimeout( () => {
					client.sendMessage(from, '20 Detik llag', text) // ur cods
					}, 10000) // 1000 = 1s,
					setTimeout( () => {
					client.sendMessage(from, '30 Detik lagi', text) // ur cods
					}, 1000) // 1000 = 1s,
					setTimeout( () => {
					client.sendMessage(from, anu.result.question, text, { quoted: mek }) // ur cods
					}, 0) // 1000 = 1s,
					break
case 'tebakgambar':
...
if (isLimit(sender)) return reply(ind.limitend(pusname))
reply(ind.wait()) 
anu = await fetchJson(`https://pencarikode.xyz/api/cita-cita?apikey=pais`, {method: 'get'}) 
  client.sendMessage(from, audio, {quoted: mek})
                await limitAdd(sender)
                break
                                        await limitAdd(sender)
					break				
...
if (args.length < 1) return reply(`「❗」Contoh : ${prefix}hartatahta botwea`)
play = body.slice(12)
reply('「⏳」PLAYBUTTON LAGI DI PROSES') 
buffer = await getBuffer(`https://api.zeks.xyz/api/splaybutton?text=${play}&apikey=BotWeA`)
client.sendMessage(from, buffer, image, {quoted: mek})
await limitAdd(sender)
break
case 'gplaybutton':
if (!isRegistered) return reply(ind.noregis())
if (isLimit(sender)) return reply(ind.limitend(pusname))
if (args.length < 1) return reply(`「❗」Contoh : ${prefix}playbutton2 botwea`)
play2 = body.slice(12)
reply('「⏳」PLAYBUTTON LAGI DI PROSES') 
buffer = await getBuffer(`https://api.zeks.xyz/api/gplaybutton?text=${play2}&apikey=apivinz`)
client.sendMessage(from, buffer, image, {quoted: mek})
await limitAdd(sender)
break
case 'naruto':
if (!isRegistered) return reply(ind.noregis())
if (isLimit(sender)) return reply(ind.limitend(pusname))
if (args.length < 1) return reply(`「❗」Contoh : ${prefix}naruto zaki`)
naru = body.slice(12)
reply(ind.wait())
anu = await getBuffer(`https://api.zeks.xyz/api/naruto?text=${naru}&apikey=apivinz`, {method: 'get'})
buffer = await getBuffer(anu.result)
client.sendMessage(from, buffer, image, {quoted: mek})
await limitAdd(sender)
break
...
if (isLimit(sender)) return reply(ind.limitend(pusname))
if (args.length < 1) return reply(`「❗」Contoh : ${prefix}logoepep HLXBOT`)
ep = body.slice(12)
reply(ind.wait())
buffer = await getBuffer(`https://api.zeks.xyz/api/epep?text=${ep}&apikey=apivinz`)
client.sendMessage(from, buffer, image, {quoted: mek, caption: ' *LOGO EPEP BURIK UDH JADI* '})
await limitAdd(sender)
break
           ...
           case 'demote':
				...
				case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `*FGX BOT*\n\n${body.slice(4)}`})
						}
